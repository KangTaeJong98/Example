...
#include <conio.h>      // getch, kbhit 함수를 사용하기 위한 헤더 파일
    ...
    while (true) {
        ...
        if (kbhit()) {
            ...
            if (ch == 224) {
                ...
                // 224인경우 한번더 getch를해서 값을 가져오고 뱀의 방향을 바꾼다.
                ch = getch();
                snake.direction = ch;
            }
        }
...
void EndScreen() {
    ...
    if (bestScore < snake.length) {
        ...
        char score[100];
        itoa(snake.length, score, 10);  // 점수를 문자열로 바꿈
